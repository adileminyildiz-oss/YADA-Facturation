  ...
  <div style="max-width:560px;margin:0 auto;padding:24px 16px">
    <div style="background:#fff;border:1px solid #e6e9f2;border-radius:14px;overflow:hidden">
      ...
      <div style="padding:20px 24px">
        ...
        ${(inv.payUrl || inv.portalUrl) ? `<div style="margin-top:18px">${inv.payUrl ? `<a href="${esc(inv.payUrl)}" style="display:inline-block;background:#2b45ff;color:#fff;text-decoration:none;font-weight:600;font-size:14px;padding:11px 22px;border-radius:10px;margin-right:8px">Payer en ligne</a>` : ""}${inv.portalUrl ? `<a href="${esc(inv.portalUrl)}" style="display:inline-block;background:#0b1020;color:#fff;text-decoration:none;font-weight:600;font-size:14px;padding:11px 22px;border-radius:10px">Voir &amp; télécharger en ligne</a>` : ""}</div>` : ""}
